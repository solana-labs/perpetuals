...
def fetch_asset_prices(assets, timestep, event):
    ...
    for asset in assets:
        file_path = os.path.join(parent_directory, 'data_CUST', f'{event}', f'{asset}-{event}-{num_hr}.csv')
...
def get_asset_volatility(assets, timestep, event):
    ...
    for asset in assets:
        file_path = os.path.join(parent_directory, f'data_CUST', f'{event}', f'{asset}-{event}-{num_hr}.csv')
        df = pd.read_csv(file_path) 
        try:
            closest_idx = (df['time']- minute).abs().idxmin()
